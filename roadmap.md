...
- [x] **2.1.1 — Generic admin CRUD framework.** ✅ Done 2026-07-16 — all 7 editors (Item, Mob, Vendor,
  ...
  separate concern).
- [x] **2.9 — Abilities, tags & effects.** ✅ Done 2026-07-17, verified in-editor ([devplan](docs/devplans/2.9-abilities-content-type.md), AB1–AB8 all as recommended). `abilities` + `ability_tags` + cooldown links + the ordered effect list (effect-composition model preserved as a typed `ability_effects` child table — `effect_type` + a shared amount/scaling-stat/scaling-factor shape covering both existing effect types; new effect types add columns when actually built, no speculative schema). **Needs client sync (like 2.2), not server-only** — `HotbarUI` reads `AbilityRegistry` on the client; new `ContentCatalog.AbilityCatalogMessage`. Web **Ability Editor** + **Ability Tag Editor**, built on 2.1.1's `ContentGrid`/`CrudModal` from day one. Also resolves a cross-item coupling with 2.10 up front: `ClassDefinition.startingAbilities` changed from `List<AbilityDefinition>` (asset refs) to `List<string>` (ability ids), so 2.10 inherits a clean id-based reference instead of a dangling asset type. **Implementation-time simplification (checked the actual `ItemDefinition` precedent, which the draft hadn't):** `AbilityDefinition`/`AbilityTag`/`DamageEffect`/`HealEffect` all stay `ScriptableObject`s (just no longer asset-authored — `AbilityRegistry.Build` constructs them via `CreateInstance` from a DB snapshot, exactly like `ItemRegistry` already does), so no plain-class/factory rewrite was needed. Retired `AbilityEditorWindow.cs`/`AbilityTagEditorWindow.cs`/`ClassContentSetup.cs` + the old `Resources/Abilities`/`AbilityTags` SO assets, faithfully seeded into the DB (`DatabaseSeeder.SeedAbilities`) with their exact original ids so existing character hotbars keep resolving. `dotnet build`/`ng build` both clean. **One real snag hit during verification, not a code bug:** `Tools/Database/Run Migrations` applies schema only — seeding is a separate step (`Tools/Database/Seed Database` or a Host start with `SeedOnStart`) — so the freshly-migrated `abilities` table was empty until seeding ran; a known, previously-documented gap in this workflow, not specific to 2.9. Once seeded, Kick/Taunt/Fire Bolt/Minor Heal all confirmed working. Verified in-editor by the user.
- [x] **2.10 — Races & classes.** ✅ Done 2026-07-18, verified in-editor ([devplan](docs/devplans/2.10-races-classes-content-type.md), RC1–RC10 all as recommended). Stat tables, XP modifiers, HP/mana formula fields, starting-ability lists (now ability ids per 2.9's AB8) → DB + web **Race Editor** + **Class Editor**. **Needs client sync, not server-only** — confirmed `RaceClassRegistry` is read client-side (`CharacterModelFactory`/`CharacterPreview`), same bucket as items/abilities. Header rows stay runtime `ScriptableObject`s built via `CreateInstance` from DB snapshots (the 2.9 AB1 pattern) — folded directly into `RaceClassRegistry` itself rather than adding separate `RaceRegistry`/`ClassRegistry` scene singletons, since it was already a plain static class. **RC4**: `ClassDefinition.weaponPropPrefab` moved off the DB-bound class onto the still-Unity-asset `CharacterRoster` (new `ClassWeaponProp` list), mirroring the `MobModelCatalog` precedent — `CharacterRoster` itself stays a Unity asset, not DB-migrated (RC5). Found + preserved a real workflow feature along the way: `CharacterPreview`'s live grip-offset tuning (edit the Inspector during Play, see the weapon move) now re-reads the roster each frame instead of caching a `ClassDefinition` ref. Faithfully seeded Human/Dwarf/Warrior/Wizard/Cleric with their exact live values (Warrior's base stats — all 75 — preserved as-is, not "corrected"). Verified in-editor by the user (one snag along the way — a stale live API process still running the pre-2.10 build caused a transient 404 on `/api/classes`; resolved by restarting the API, not a code issue). Two new Documentation-tab guides (**Abilities**, **Races & Classes**) added the same session, grounded in the real code — the Races & Classes guide leads with the load-bearing gotcha this item itself surfaced: authoring a race/class in the web editor is necessary but not sufficient to make it selectable at character creation, since the create cascade + server-side whitelist both read the Unity-only `CharacterRoster` asset, not the DB tables directly.
- [x] **2.11 — Seed / export / content versioning.** ✅ Done 2026-07-18, verified in-editor ([devplan](docs/devplans/2.11-seed-export-versioning.md), SE1–SE5 all as recommended). **SE1: reproducible seed already exists** — `DatabaseSeeder` is idempotent and confirmed sufficient, no rebuild needed. **SE2/SE3 (the real deliverable): a generic, schema-driven content export/import** — enumerates every DB table via `information_schema` minus a small player-state exclude-list (accounts/characters/character_*), so it needs zero updates as future content types ship (unlike a hand-maintained per-type list); import wipes + reloads inside one transaction (a combined multi-table `TRUNCATE ... CASCADE` + `session_replication_role = replica` for FK-order-independent inserts, then resets serial sequences). Lives in `Tools/Database/Export Content...` / `Import Content...` alongside the existing DB tooling, not a shell/pg_dump wrapper. Every value round-trips as an explicit canonical string (not a raw JSON number/bool) keyed to its captured Postgres column type, avoiding JSON type-inference ambiguity on import. **SE4: a real, grounded gap found while scoping this** — `races`/`classes`/`ability_tags` were missing the `updated_at` column every other content header table already has; fixed via migration `0025`. A full change-attribution audit log is explicitly deferred to 6.3 (real auth) — there's no "who" to log against yet. `dotnet build` clean. **Verified via a real round-trip**: migrations applied clean against a fresh throwaway Postgres container (24 files — there is no `0003`, so 24 is the correct full count, not a partial run); exported content from the real dev DB; imported into the throwaway DB; row counts matched exactly across all 29 content tables (abilities/items/mobs/factions/loot/spawn/xp/etc). One real gotcha hit and worth remembering: `Database.OpenConnection()` caches the resolved connection string in a `static` field for the life of the Editor's script domain, so editing `db.config.json` to retarget a different DB has no effect until a script recompile forces a domain reload — silently reused the old connection twice during this test. **Closes M2's originally-scoped 2.1–2.11 list.**
...
> M4 regardless. Content work here (3.5) is unaffected — it already gets by on today's placeholder
> faction aggro.
> Sequencing (revised 2026-07-17): **3.6 (zone content & decoration pass) is retired from this milestone
> and moved to its own milestone, M7** — the remaining decoration/population work is being itemized
> POI-by-POI (one zone/point-of-interest per roadmap item, built one at a time) rather than staying one
> big undifferentiated item. See M7.
...
> Sequencing (revised 2026-07-26, ideation session): **gameplay depth is the priority lens going forward**,
> run concurrently with (not blocked on) **M7's content build-out** — the user's brother now has web-tooling
> access (5.11) and can help populate zones in parallel with depth work. **6.3 (transport security/real auth)
> and 6.5 (load testing) are explicitly pushed to a post-first-3-zones backlog** — the near-term audience is
> the user + brother nightly, plus family (including kids) on weekends, not an open/public player base, so
> hardening doesn't buy much yet. Real threat/aggro (still the 1:1-damage placeholder; retired from 3.4,
> folded into 5.1's scope note but never itself devplanned) is the standing gap most of the rest of this
> milestone conceptually leans on. **Within-M5 order decided 2026-07-26: 5.3 (player grouping) → real
> threat/aggro → 5.2 (level 1→2 polish round)** — tie-broken on where Claude Code adds the most leverage
> per session, not build-dependency order: grouping and threat are real systems/code work (Claude-heavy
> leverage), while 5.2 is subjective feel/pacing tuning done largely by hands-on playtesting (low
> Claude-code leverage) — so it's pushed to last despite being the smallest/lowest-risk lift. The user has
...
- [x] **5.3 — Player grouping.** ✅ Done 2026-07-26, reviewed GP1–GP11 one at a time
  ...
  `EnemyAI`'s existing threat-list entries (already a 1:1 damage mirror) by party (an ungrouped player counts
...
- [x] **5.10 — Dedicated server build spike.** ✅ Done 2026-07-25 ([devplan](docs/devplans/5.10-dedicated-server-build-spike.md)).
  *(Added 2026-07-17, ahead of M6 on purpose.)* Proved out a real standalone/dedicated Unity build —
  de-risks 6.2 (dedicated server + hosting), 6.4 (build pipeline), and 6.5 (load testing) all at once.
  The original 2026-06-21 crash (corrupted `level0`) was root-caused via a long elimination process
  (ruled out: stale build folder, path-with-spaces, Norton AV, D3D11 vs D3D12, Unity's incremental build
  cache) down to a genuine engine-level bug packing/loading multiple additive scenes in this environment
  — confirmed by matching historical Unity Issue Tracker reports, not just this project's symptom.
  **Fixed by reworking the HUD/menu UI layer from an additively-loaded scene into a prefab**
  (`Resources/UI/HudRoot.prefab`, instantiated directly by `UIManager` instead of `SceneManager.
  LoadSceneAsync`) — sidesteps the buggy mechanism entirely rather than chasing its exact trigger,
  matching how every other asset type in this project already loads (Resources.Load + Instantiate).
  Two further real bugs surfaced once the client was actually usable, both found and fixed: (1) Hotbar's
  slot components silently lost their script reference on every build — root cause was `HotbarUI.cs`
  declaring two MonoBehaviour classes in one file (a known Unity gotcha for a class not matching its
  filename), fixed by giving `HotbarSlotUI` its own file; (2) `LocalPlayer.Spawned` fired with 0
  subscribers, silently breaking PlayerFrame/TargetFrame/Hotbar/Equipment/Inventory/Vendor UI binding
  all at once — the exact trigger wasn't fully pinned down (this is genuinely the first time a real
  standalone client+server pair, not Editor/Host/MPPM, has exercised this binding path), so rather than
  keep chasing it, added a self-healing `Update()` fallback to all six affected panels that re-binds
  whenever `LocalPlayer.Current` is set but the panel hasn't bound yet. **Also found: the headless
  server and standalone client must be rebuilt together any time either changes** — even a client-only
  change shifts `SampleScene`'s Mirror scene-object hashing enough to desync an old server. Full DS5
  checklist (register→login→create→spawn→move→target→combat→loot→camp→relog) confirmed working end to
  end against two genuinely separate, independently-built processes.
- [x] **5.11 — Web/API CI/CD + AWS hosting.** ✅ Done 2026-07-22. 🟢 Approved 2026-07-18, reviewed CD1–CD9 one at a time with the user ([devplan](docs/devplans/5.11-web-api-cicd-aws-hosting.md)). All decisions confirmed as originally written except CD5 (briefly reconsidered — SSH open + allowlisted for convenience — then reverted back to closed entirely); CD8 (manual migrations for now) flagged a portable non-Unity migration runner as an expected fast-follow, not just a maybe. *(Added 2026-07-18, sequenced right after 5.10 — both are pre-M6 infra work pulled forward on the user's call; nothing in 5.1–5.9 or 7.x has any bearing on it.)* Pulls roadmap **6.1** ("Deployed Postgres on AWS") forward. Goal: get `/web` + `/api` + Postgres reachable somewhere other than localhost so the user's brother (no SE background, out of state) can use the web content editors without either of them running the stack locally — explicitly **not** the Unity dedicated game server (5.10/6.2), separate infrastructure, no coupling. Grounded directly in the user's existing sibling AWS app (`family-cookbook`: Angular→S3+CloudFront, Express API→Lambda+API Gateway, DB→Aurora DSQL, GitHub OIDC deploy) — reuses its already-registered GitHub OIDC provider and its "zero long-lived AWS secrets" ethos, but **deliberately departs from its hosting shape** on two points: (1) plain Postgres via Docker, not Aurora DSQL — this session's own 2.11 export/import tool depends on `pg_get_serial_sequence`/`setval`, which DSQL doesn't cleanly support, and Ueq's 25 existing migrations all lean on `BIGSERIAL`; (2) both `/web` and `/api` served directly off one Lightsail box via Caddy, not S3+CloudFront+API Gateway — `family-cookbook` is a public family site with no access restriction, while Ueq's tool is intentionally IP-allowlisted to two known people, and fighting a CDN's public-by-design reach with a WAF/CloudFront Function is more moving parts than just firewalling one box. Single Lightsail instance ($10/mo tier — the $5 tier's 0.5 GB RAM is too tight once a future game-server process joins Postgres+API on the same box), Caddy for automatic Let's Encrypt HTTPS, Lightsail firewall IP-allowlisted (SSH closed entirely — admin access via Lightsail's browser-based SSH instead). Deploy mechanism keeps the zero-secrets ethos despite the different hosting shape: GitHub OIDC → uploads build artifacts to a private S3 bucket → `aws ssm send-command` runs a deploy script on the instance (pull + unpack + restart services) — no stored SSH key, no open SSH port, using Lightsail's documented SSM support. A separate, always-green CI workflow (build validation only, no AWS credentials) runs on every push/PR: `dotnet build` + `ng build` + a migration-order sanity check against a throwaway Postgres container. One explicitly-scoped gap: applying schema migrations to the deployed DB stays **manual** (point the Editor's config at the target DB, `Run Migrations`, `Import Content...` — the exact playbook just verified this session for 2.11) since `/api` has no migration-running code of its own; a portable non-Unity migration runner is flagged as a reasonable future item, not built here. Reserves room (CD9) for the eventual Unity dedicated server as a fourth co-located process; whether it stays co-located or splits onto dedicated capacity is explicitly a 6.5 (load testing) decision, not this one. **Closed 2026-07-22.** All 13 runbook steps complete: one-time AWS setup (Lightsail instance, static IP, firewall, sslip.io, Docker Postgres, Caddy/cert, SSM hybrid activation, S3 deploy bucket, OIDC deploy role, GitHub secrets/variables), `ci.yml` + `deploy.yml` both green end-to-end (after fixing a real pre-existing `.gitignore` bug — blanket `*.csproj` was silently excluding `api/Ueq.ContentApi.csproj` from git, so it had never actually been committed — plus several real deploy-pipeline bugs: region propagation, Lightsail's own baked-in identity shadowing the hybrid-activation role so the instance needed presigned URLs instead of its own AWS credentials, presigned-URL text-substitution corruption fixed by rebuilding the SSM command via `jq`, and a shebang-line-ordering bug that made SSM run the script under plain `sh` instead of bash — see 2026-07-22's CLAUDE.md entry for the full debugging log). Step 13 verified: unauthenticated 401, wrong-invite-code 401, and the site's real usage this session (login + browsing the populated content grids) stands in for the browser/cert checks. The deployed DB was found empty post-deploy (root cause undetermined) and re-populated via a fresh export/import — done through an **SSM port-forwarding tunnel** (`aws ssm start-session ... AWS-StartPortForwardingSession`) rather than the firewall-opening/Docker-binding-widen dance step 8 originally used; this is also the answer to the DBeaver-access want flagged 2026-07-20. **Fast-follow born from this session's friction, implemented same-day:** `Database.OpenEditorConnection()` (all `Tools/Database/*` menu commands now always re-read `db.config.json` fresh — no more Editor restart needed to switch DB targets) + `db.config.json` gained a **named-profile format** (`"active": "local" | "remote-tunnel"`, one-line switch instead of hand-editing host/port/password each time), backward-compatible with the old flat format. Also closes **6.1** early (see below).

- [ ] **5.12 — Sky & day/night cycle (with lunar phases & stars).** *(Added 2026-08-04.)*
  🟠 Implemented 2026-08-05, first real in-editor test done — one bug found + fixed, one feature
  cut, remaining verification pending
  ([devplan](docs/devplans/5.12-sky-day-night-cycle.md), decisions DC1–DC10).
  Server-authoritative day/night cycle (rotating sun, stylized gradient skybox, shifting ambient)
  + a slower cosmetic lunar phase cycle, rendering identically for every client regardless of
  zone — grounded in the fact that a Directional Light's effect is rotation-only, so one global
  sun rig in the base scene lights every zone correctly despite their large world-space offsets,
  with no per-zone duplication needed. Synced via Mirror's shared `NetworkTime` (one-time
  constants on connect, each client computes locally) rather than a per-frame SyncVar, matching
  4.2's precedent. Lunar cycle is **cosmetic-only for this pass** (user decision) — exposes a
  `CurrentLunarPhase` read hook for a future content item to opt into gameplay effects later,
  without this devplan touching `SpawnTable`/`SpawnPoint`. **Moon is fully procedural** (user
  decision, DC5) — a custom `Ueq/ProceduralMoon` shader draws phase via a single continuous
  spherical dot-product formula, needing zero art assets. **Stars folded in** (user decision,
  DC10) — a noise-based star field baked into the same sky shader, night-only, zero new assets.
  Adds a per-zone `usesDayNightCycle` opt-out on `ZoneDefinition`, anticipating Grukmar's Deep
  (7.3) wanting static dungeon lighting instead. **Clouds (DC9) were built, then cut** after the
  first real in-editor test — no owned Synty pack had a cloud prop, so the procedural fallback (a
  few overlapping spheres) was always what rendered, and it looked bad ("atrocious"); `CloudField`
  deleted rather than iterated on, revisit only with real cloud art. That same test also caught a
  real bug — the moon was tracking the sun's own arc (visible by day, hidden by night) instead of
  the opposite — root-caused to conflating a Directional Light's ray-travel-direction convention
  with a visible object's position-offset convention (they're inverses); fixed by reusing the
  sun's own angle formula unmodified rather than adding the originally-assumed +180 offset. See
  the devplan's "Verification round 1" log for the full derivation. **Still pending**: DC7's
  per-zone opt-out flag (flagged unverified-assumption risk) and the cross-zone simultaneity
  check.

## M6 — Deploy & Scale  *(AWS)*

> Goal: move from local hobby setup toward a real server.
> Sequencing (revised 2026-07-26, ideation session): **6.3 and 6.5 are deferred to a backlog**, revisited
> once the first 3 zones (Creslin's Field / Thornwood / Grukmar's Deep — M7) are content-filled and fully
> consumable by clients — see the M5 sequencing note for the reasoning.
...
- [x] **6.2 — Dedicated server build + hosting.** ✅ Done 2026-07-26 ([devplan](docs/devplans/6.2-dedicated-server-build-hosting.md), decisions DH1–DH8). *(Added 2026-07-25 right after 5.10 closed; implemented + verified over the real internet the next day.)* Scope: get the real Unity game server running as a fourth co-located process on the existing Lightsail box (per 5.11's CD9), reachable by a real client over the internet instead of localhost. Builds directly on 5.10 (proved a real standalone build works, just Windows so far) and 5.11 (the box, Postgres, and deploy pipeline already exist). Key new ground: this is the project's **first-ever Linux build** (the Lightsail box is Ubuntu; 5.10 was Windows-only) and the first use of Unity's purpose-built Dedicated Server target (5.10 explicitly deferred that switch to here). Explicitly excludes client distribution (6.4) and transport encryption/hardened auth (6.3) — both stay separate. Installed Unity Hub's Linux Dedicated Server Build Support module (project's real installed Editor is 6000.4.12f1, not the setup doc's stated 6000.4.0f1); new `Tools/Build/Build Linux Dedicated Server` in `ServerBuildTools.cs`; `deploy/ueq-gameserver.service` (mirrors `ueq-api.service`); `deploy/deploy.sh` + `.github/workflows/deploy.yml` extended to ship/restart the gameserver alongside web/api; `docs/DEPLOY-AWS.md` gained §14. Walked the user through the AWS-side steps live in chat (firewall UDP 7777, instance setup, S3 upload, deploy trigger — this sandbox's AWS CLI can't TLS-validate against AWS, so all of it ran in the user's own terminal/browser SSH). **Four real bugs found and fixed along the way, not guessed:** (1) a Lightsail browser-SSH heredoc paste corrupted the console installing the systemd unit — recovered, rebuilt via sequential `echo | sudo tee -a` appends instead; (2) the gameserver artifact's first deploy failed — PowerShell's `Compress-Archive` writes backslash path separators Linux's `unzip` can't parse for nested folders, switched the whole pipeline to `tar`/`.tar.gz`; (3) building the Linux Dedicated Server left `EditorUserBuildSettings.standaloneBuildSubtarget` stuck on Server globally (persists cross-session) and exposed that this project's `MIRROR` scripting-define symbol had only ever been configured for the Standalone platform group, never Server — so the very next build (a Windows client) failed to even compile; fixed by adding a matching `Server:` define entry to `ProjectSettings.asset` and having each build method explicitly reset the subtarget; (4) a brand-new character fell through the world on first spawn — a new `Tools/Zones/Audit Start Positions` tool (kept) found 13 duplicate `NetworkStartPosition` objects, all leftover from repeated `Setup All` runs over many past sessions, frozen at a pre-terrain-reshape coordinate now ~8 units below the real (reshaped, 3.1.9) ground — Mirror's random pick meant a 13-in-14 chance of spawning embedded in solid terrain, invisible in all prior Editor/Host/MPPM testing since that never exercised a genuinely fresh `GetStartPosition()` pick the way a real new client+account did; fixed by removing the 13 duplicates (`Tools/Zones/Clean Duplicate Start Positions`, also kept) and making `Setup All`'s start-position creation idempotent + ground-snapped so it can't recur. **User confirmed the full register→login→create→spawn→move loop working over the real internet against the AWS-hosted server and Postgres.**
- [ ] **6.3 — Transport security + real auth** (the deferred security work: encryption, hardened login).
- [x] **6.4 — Build pipeline / client distribution + launcher.** ✅ Done — verified over the real internet 2026-07-26 (checkbox sync fixed 2026-07-26; devplan status line was stuck at 🟠, roadmap wasn't updated at the time even though CLAUDE.md's session log already called it closed) ([devplan](docs/devplans/6.4-build-pipeline-launcher.md), decisions BP1–BP6). Scoped down from the roadmap's older "launcher/patcher" framing — the actual driver is speeding up content-development iteration between the user and their brother (no SE background, can't build the client himself), not distributing to an unknown public player base. **User pushed back on the original "no launcher" call (BP6)** — landed on a simple always-full-redownload auto-launcher, not true delta/binary-diff patching (deferred until it's an actual pain point). Built: `BuildInfo.cs` + `Tools/Build/Stamp New Build Id` (a deliberately separate stamping step so client/server built moments apart don't get two different ids); a build-id mismatch check in the login handshake (`AccountAuthenticator`, new `AuthCode.VersionMismatch`); a small self-contained `launcher/Ueq.Launcher/` (.NET WinForms, single-file publish); reused 6.2's S3-presign/SSM deploy pipeline to ship `client.zip`/`UeqLauncher.exe` off a new `/downloads/*` Caddy path; a new "Build & Deploy" guide in the Documentation tab; a new authenticated **Play** tab in the Angular site (`play-page.ts`, now the default landing view on login) hosting the download. **Real-world verification surfaced four more real bugs, all fixed:** (1) a Lightsail browser-SSH heredoc corrupted the console installing the systemd unit (same known failure mode as prior sessions) — recovered via sequential `echo | sudo tee -a` appends; (2) the first real client download 404'd — root cause was that `deploy/Caddyfile` edits in the repo were never actually part of the automated deploy pipeline (a one-time manual copy from the original setup runbook), so the live Caddy config had no `/downloads/*` route — fixed + documented as a standing gotcha; (3) hand-pasting a presigned URL for the launcher exe corrupted in the browser-SSH console — fixed by wiring it into the automated `deploy.sh`/`deploy.yml` pipeline instead; (4) **the substantive one** — the staleness check compared a `version.txt`/`BuildInfo` stamp that's only bumped for deliberate compatibility releases, so a routine client-only rebuild (like the same-session Local/Deployed toggle) would never trip it, leaving the launcher thinking it was always up to date — fixed at the design level by having the launcher fingerprint `UeqClient.zip` via its own HTTP `ETag`/`Last-Modified` instead of a separately-maintained version file, removing `version.txt`/`VERSION_URL` from the pipeline entirely. Also replaced the title screen's free-text server-address field with a Local/Deployed toggle (defaults to Deployed). **Fully verified end-to-end**: fresh launcher run → downloads → extracts → launches → title screen defaulted to Deployed — confirmed by the user.
- [ ] **6.5 — Load testing + zone-server architecture.**

---

## M7 — Content Pass  *(zone decoration & population, itemized POI-by-POI)*

> Goal: the remaining "make the world feel built" work — decoration, points of interest, named NPCs,
> themed population — broken into **individual roadmap items, one per zone or point of interest**, instead
> of one big undifferentiated pass. *(Was 3.6 — retired from M3 and moved here 2026-07-17, specifically so
> it could be itemized this way.)* Each item below still gets its own devplan before it's built, same as
> everywhere else on this roadmap; the difference here is granularity — a POI is worked start-to-finish,
> then the next one is picked. **Sequencing (revised 2026-07-26, ideation session): runs concurrently with
> M5 gameplay depth**, not after it — the user's brother has web-tooling access (5.11) and can pick up
> content items in parallel with depth work on the Unity side. **The three items below are the current zone-level placeholders; expect them
> to get split further (7.1.1, 7.1.2, …) as specific points of interest inside each zone are identified —
> add new dotted-decimal items as they come up rather than trying to enumerate the whole list up front.**

- [ ] **7.1 — Creslin's Field: remaining decoration & POIs.** The 3.1.9/3.1.10/3.1.11 starter-zone vertical
  covered shaping + a starter encounter set for the playable core; this is the rest of the ~1500×1500u
  field — broader hand-placed Synty dressing, additional named NPCs, and themed points of interest along
  the edges (flagged as deferred in 3.1.9).
- [ ] **7.2 — Thornwood: remaining decoration & POIs.** Picks up what 3.5 explicitly deferred: the design
  doc's full 4-tier (level 6–15) difficulty gradient, named encounters (Shaman/Warchief), the wildlife/
  zone-sweeper predator (blocked on Synty art — no owned predator body in any imported pack yet), and
  atmosphere/lighting pass.
