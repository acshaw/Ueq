...
- [ ] **5.10 — Dedicated server build spike.** *(Added 2026-07-17, ahead of M6 on purpose.)* Prove out a
  real standalone/dedicated Unity build before any AWS work depends on one — de-risks 6.2 (dedicated
  server + hosting), 6.4 (build pipeline), and 6.5 (load testing) all at once. **Known blocker to
  diagnose:** the one standalone-build attempt so far (2026-06-21) crashed instantly with a corrupted
  `level0` (`Player.log`), reproduced on a clean rebuild, and was parked in favor of Multiplayer Play Mode
  rather than root-caused — suspects noted at the time were the project living under a synced folder
  mangling build output, or a Unity 6.0.4 build quirk. Scope: get a `-batchmode -nographics` headless
  server build booting, connecting to the Dockerized Postgres, and accepting real (non-Editor) client
  connections — the actual proof a "dedicated server" is deployable at all. Needs its own devplan; not
  itemized further yet.
...
> then the next one is picked. **The three items below are the current zone-level placeholders; expect them
