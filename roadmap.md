...
- [ ] **2.2 — Items (reference content type).** First end-to-end vertical: `items` table + migration, server loader replacing `ItemRegistry`'s Resources path, web **Item Editor** (identity, stats, equip slot + bonuses, weapon stats, economy). Validates the full pattern against the most-connected data (inventory, loot, vendor, equipment). Everything after copies this shape.
- [ ] **2.3 — Quests (new content type).** Quest **data model** + web **Quest Editor**: objectives, trigger keywords, faction gates, and a reward bundle (XP / items / faction / currency). Greenfield — quests don't exist yet as data. Runtime granting is **3.2** (keyword reward action); this item delivers the authorable definitions 3.2 consumes.
- [ ] **2.4 — Loot tables + XP thresholds.** Pure-data quick wins on the 2.2 pattern: `loot_tables` (+ entries/coin tiers) and the XP table move to DB + web editors; retire `Resources/XpTable.asset` and the Loot Table SO path.
- [ ] **2.5 — Abilities, tags & effects.** `ability_definitions` + `ability_tags` + cooldown links + the ordered effect list (effect-composition model preserved; effects referenced by type + params). Web **Ability Editor** + **Ability Tag Editor**. Uses the 2.1 convention for anim-trigger + prefab bindings.
- [ ] **2.6 — Races & classes.** Stat tables, XP modifiers, HP/mana formula fields, starting-ability lists → DB + web **Race & Class Editor**. Feeds `SetRaceClass` and character creation (1.5) once migrated.
- [ ] **2.7 — Mobs & NPC wiring.** `MobDefinition` (identity / combat / movement / AI / faction / rewards / vendor) + spawn tables/timers + conversation keyword sets + the race→faction defaults table → DB + web editors. Consolidates the old per-tool "NPC config / spawn / conversation / faction defaults" goals into one web surface.
- [ ] **2.8 — Seed / export / content versioning.** Reproducible DB seed, content export/import (sharing content between dev DBs / promoting to AWS), and a content-change audit trail as the library grows.
