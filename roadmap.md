...
- [ ] **2.10 — Races & classes.** 🟠 Implemented 2026-07-18, pending in-editor verification ([devplan](docs/devplans/2.10-races-classes-content-type.md), RC1–RC10 all as recommended). Stat tables, XP modifiers, HP/mana formula fields, starting-ability lists (now ability ids per 2.9's AB8) → DB + web **Race Editor** + **Class Editor**. **Needs client sync, not server-only** — confirmed `RaceClassRegistry` is read client-side (`CharacterModelFactory`/`CharacterPreview`), same bucket as items/abilities. Header rows stay runtime `ScriptableObject`s built via `CreateInstance` from DB snapshots (the 2.9 AB1 pattern) — folded directly into `RaceClassRegistry` itself rather than adding separate `RaceRegistry`/`ClassRegistry` scene singletons, since it was already a plain static class. **RC4**: `ClassDefinition.weaponPropPrefab` moved off the DB-bound class onto the still-Unity-asset `CharacterRoster` (new `ClassWeaponProp` list), mirroring the `MobModelCatalog` precedent — `CharacterRoster` itself stays a Unity asset, not DB-migrated (RC5). Found + preserved a real workflow feature along the way: `CharacterPreview`'s live grip-offset tuning (edit the Inspector during Play, see the weapon move) now re-reads the roster each frame instead of caching a `ClassDefinition` ref. `dotnet build`/`ng build` both clean. Faithfully seeded Human/Dwarf/Warrior/Wizard/Cleric with their exact live values (Warrior's base stats — all 75 — preserved as-is, not "corrected"). **NEXT (user, in editor): recompile → `Tools/Database/Run Migrations` (0024) → `Tools/Database/Seed Database` → host (expect `[Content] Loaded 2 race(s)` + `3 class(es)`) → full character-creation regression (all 7 gender/race/class tuples still create + spawn with correct body/stats/hotbar/weapon prop — the real risk surface, since 3.1.4–3.1.6 built a lot on top of `RaceClassRegistry`) → `Tools/Character/Build Character Roster` if weapon props don't show (roster asset was hand-pre-populated, should already work). Then mark 2.10 ✅ → 2.11.**
